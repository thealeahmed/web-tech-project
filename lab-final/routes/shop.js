...
router.get("/cart", async function (req, res, next) {
  // Task 1 & 4 & 5: Using session-based cart instead of cookies for better security and persistence
  let cart = req.session.cart || [];

  // Task 4: Recalculate cart total on the server
  const total = cart.reduce(
    (runningTotal, item) => runningTotal + (Number(item.price || 0) * (item.quantity || 1)),
    0
  );

  res.render("site/cart", { products: cart, total, layout: "layout" });
});
...
router.get("/add-cart/:id", async function (req, res, next) {
  ...
  const existingProductIndex = cart.findIndex(item => item._id == req.params.id);
